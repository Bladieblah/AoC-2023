...
fn find_sols(time: &usize, dist: &usize) -> usize {
  let d = ((time.pow(2) - 4 * dist) as f64).sqrt();
  ((*time as f64 + d) / 2. - 0.000001).floor() as usize - ((*time as f64 - d) / 2. + 0.000001).ceil()  as usize + 1
}
...
let p1 = time.iter().zip(dist.iter()).map(|(t, d)| find_sols(t, d)).product();
